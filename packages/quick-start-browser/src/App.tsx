...
function App(){
  ...
  const queryClient = React.useMemo(() => { try {
    return config.applicationId && config.kustoUri ? new Client(KustoConnectionStringBuilder.withUserPrompt(config.kustoUri,
        {
  ...
  const ingestClient = React.useMemo(() => { try {
    return config.applicationId && config.ingestUri ? new IngestClient(KustoConnectionStringBuilder.withUserPrompt(config.ingestUri,
      {
        redirectUri: window.location.href,
        clientId: config.applicationId
      })) : null     } catch (error) {
  ...
  return (
    ...
    style={{padding:20}}
      ...
      <UpperFields config={config} setConfig={setConfig} tableCreated={state.tableCreated} setTenant={(tenantId:string) => setState({...state, tenantId})}></UpperFields>
      <Switch
      ...
      {queryClient && config.useExistingTable === false &&       
        <CreateTable databaseName={config.databaseName} queryClient={queryClient} tableCreated = {state.tableCreated} tableName={config.tableName} tableSchema={config.tableSchema} setTableCreated={(tableCreated:boolean)=>setState({...state, tableCreated})} ></CreateTable>}
      <div style={{ display: "grid", maxWidth: 100}}>
        ...
        style={{width: "100", marginTop:10, marginBottom:10}}
          ...
          onOptionSelect={(_, option) => { setState({ ... state, ingestOrQuery: option?.optionValue as IngestOrQuery}) }}
        >
      ...
      {state.ingestOrQuery == "Ingest" ? 
       ...
       <QueryFlow useExistingTable={config.useExistingTable} database={config.databaseName} table={config.tableName} queryClient={queryClient} tableCreated={state.tableCreated} ></QueryFlow>}
     </FluentProvider>
  )
}

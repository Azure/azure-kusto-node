...
export const CreateTable: React.FunctionComponent<CreateTableProps> = ({tableCreated, tableName, tableSchema, queryClient, databaseName, setTableCreated}) => {
    ...
    return (
      ...
      (<div style={{paddingTop:10}}>
        ...
        />
        <Button disabled={state.ongoing || !state.tableSchema} onClick={() => {
          setState({ongoing:true, tableSchema});
          const command = `.create table ${tableName} ${state.tableSchema}`;
          queryClient.executeMgmt(databaseName, command)
            .then(
              (_) => {

...
// Usage: import main into your code, or this package index.js for example.
...
// const appId = "<aadAppId>";
const database = "fast";
const table = "TestTable2";
const clusterName = "devdevon.westeurope";
...
export const main = async (): Promise<void> => {
    const interactiveBrowserAuthOptions = {
        ...
        clientId: appId ?? "c8dbda70-81cc-49e0-aa47-6133d7154fe3", // Read above regarding which app to use
        redirectUri: redirectUri ?? "http://localhost:3000"
    };
    const kcsb = KustoConnectionStringBuilder.withUserPrompt(
        `https://${clusterName}.dev.kusto.windows.net`,
        interactiveBrowserAuthOptions);
    const queryClient = new KustoClient(kcsb);
    const kcsbIng = KustoConnectionStringBuilder.withUserPrompt(
        `https://ingest-${clusterName}.dev.kusto.windows.net`,
        interactiveBrowserAuthOptions);
